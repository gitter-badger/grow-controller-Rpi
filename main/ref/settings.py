...
def heat():
    global heat1
    heat1 = 1  # is a heater installed, 0 = no / 1 = yes
def cooling():
    global cool1
    cool1 = 1  # is a AC unit installed, 0 = no / 1 = yes
def webgui():  #  requires a Adafruit.IO account(free)
    global enable1, key1
    enable1 = 1   # webGui enable #  ## 0 = no / 1 = yes
    key1 = 'your_adafruit_IO_key'

def boot():   # what to turn on at boot, NOT FINISHED , ( 0 = off / 1 = on / 2 = on if light on / 3 = on if light off )
    global boot1, boot2, boot3, boot4
    boot1 = 0  # ballast
    boot2 = 0  # ballfan
    boot3 = 0  # heater
    boot4 = 0  # ocfan
